...
export async function health() {
  ...
  // Storage check - skip external ping if in local mode
  if (ENV.storageMode === "local" || ENV.forceStorageLocal) {
    out.storage = "healthy";
  } else {
  ...
  // Embeddings check - skip if debug short-circuit is enabled
  if (ENV.debugUploadShortCircuit) {
    out.embeddings = "healthy";
  } else {
